...
# MAGIC %md
# MAGIC ## B. Create the Databricks Job with Notebooks, Python Files and a Lakeflow Declarative Pipeline
# MAGIC
# MAGIC 1. In this section, we will create the job for our project. The job will contain the following tasks:
# MAGIC     - Unit tests
# MAGIC     - Lakeflow Declarative Pipeline with the ETL pipeline and integration tests 
...
# MAGIC %md
# MAGIC 2. During development, it's beneficial to build out your Workflow and/or Lakeflow Declarative Pipelineusing the UI. The UI provides an easy way to build your desired job, and it generates the necessary JSON or YAML files to start automating your deployment across different environments.
# MAGIC
...
## Confirm the pipeline from the previous demonstration exists. If not, create the Lakeflow Declarative Pipeline and store the ID
...
# MAGIC
# MAGIC    **NOTE:** If the Lakeflow Declarative Pipeline has already completed, simply select the pipeline link.
# MAGIC
# MAGIC - c. Notice the pipeline executes (will execute) the ETL pipeline we created earlier.
# MAGIC
# MAGIC - d. Close the Lakeflow Declarative Pipeline.
# MAGIC
# MAGIC
...
# MAGIC - How do I configure the necessary variables for the Lakeflow Declarative Pipeline during each deployment?
...
# MAGIC ### Next Steps: 
# MAGIC
# MAGIC In this course, you explored the essentials of CI/CD with a focus on continuous integration (CI) in Databricks. Next, you’ll want to dive into the other half of the DevOps pipeline, continuous deployment (CD) by learning how to deploy assets using **Databricks Asset Bundles (DABs)**.
# MAGIC
# MAGIC #### DABs Documentation [What are Databricks Asset Bundles?](https://docs.databricks.com/en/dev-tools/bundles/index.html) 
# MAGIC
